...
void construct_matrices(int *n_ptr, int *m_ptr, int *l_ptr, int **a_mat_ptr, int **b_mat_ptr){
	...
	if(world_rank == 0) {
		...
		for(i = 1; i < world_size; i++) {
			MPI_Send(n_ptr, 1, MPI_INT, i, 0, MPI_COMM_WORLD);
	...
	*b_mat_ptr = (int*)calloc(bs, sizeof(int));
	...
	if(world_rank == 0) {
		for(i = 2; i < world_size; i += 2) {
			MPI_Send(*a_mat_ptr, as, MPI_INT, i, 0, MPI_COMM_WORLD);
			MPI_Send(*b_mat_ptr, bs, MPI_INT, i, 0, MPI_COMM_WORLD);
		}
	}
	if(world_rank % 2 == 0){
		if(world_rank != 0) {
			MPI_Irecv( *a_mat_ptr, as, MPI_INT , 0 , 0 , MPI_COMM_WORLD , &request[0]);
			MPI_Irecv( *b_mat_ptr, bs, MPI_INT , 0 , 0 , MPI_COMM_WORLD , &request[1]);
			MPI_Waitall(2, request, MPI_STATUSES_IGNORE);
		}
		if(world_rank + 1 < world_size) {
			MPI_Send(*a_mat_ptr, as, MPI_INT, world_rank + 1, 0, MPI_COMM_WORLD);
			MPI_Send(*b_mat_ptr, bs, MPI_INT, world_rank + 1, 0, MPI_COMM_WORLD);
		}
	}
	else {
		MPI_Irecv( *a_mat_ptr, as, MPI_INT , world_rank - 1 , 0 , MPI_COMM_WORLD , &request[0]);
		MPI_Irecv( *b_mat_ptr, bs, MPI_INT , world_rank - 1 , 0 , MPI_COMM_WORLD , &request[1]);
	}
}
...
void matrix_multiply(const int n, const int m, const int l, const int *a_mat, const int *b_mat){
	...
	if(world_rank == 0){
		...
		{
			buffer[source-1] = (int*)malloc(len, sizeof(int));
			MPI_Irecv( buffer[source-1], len, MPI_INT, source, 0, MPI_COMM_WORLD, &requests[source-1]);
		}
		MPI_Waitall(world_size-1, requests, MPI_STATUSES_IGNORE);
		for (int source = 0; source < world_size-1; source++) { 
			for(int j = 0; j < len; j++) {
				ans[j] = ans[j] + buffer[source][j];
			}
		}
		for(i = 0 ; i < n ; i++){
			c_idx = i * l;
